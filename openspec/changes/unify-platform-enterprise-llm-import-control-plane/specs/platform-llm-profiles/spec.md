...
### Requirement: 平台管理员动态控制回答策略
系统 SHALL 在每个 LLM profile 上保存由平台管理员控制的低风险通用问答和 FAQ 快速直答开关，并在下一次访客 Chat 请求中即时采用；企业角色不得读取写入入口以外的密钥信息，也不得修改这些开关。

#### Scenario: 开启低风险通用问答
- **WHEN** 平台管理员为当前主配置开启低风险通用问答
- **THEN** 后续缺少企业证据的低风险自由问题可以调用当前模型回答，且无需重启服务

#### Scenario: 高风险门禁不被放宽
- **WHEN** 低风险通用问答已开启但访客询问价格、承诺或其他高风险内容
- **THEN** 系统仍要求企业证据或拒答，不得因为开关开启而自由生成

#### Scenario: 开启 FAQ 快速直答
- **WHEN** 平台管理员开启 FAQ 快速直答且问题高置信命中已发布 FAQ
- **THEN** 系统直接返回该 FAQ 的标准答案、不调用模型，并且无需重启服务

#### Scenario: 默认保持严格模式
- **WHEN** 既有 profile 完成迁移或管理员新建 profile 时未显式开启开关
- **THEN** 两项开关均为关闭，系统保持现有严格证据门和普通 FAQ/RAG 路径
